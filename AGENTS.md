...
| `elements/` | HUD 元素基类 `HudModule` 与具体实现（`Notifications`、独立信息 HUD（`FPS`、`TPS`、`Ping`、`Speed`、`Coordinates`、`Game Mode`、`Main Hand Item`、`Off Hand Item`、`Looking At`、`Breaking Progress`、`Server`、`Weather`、`Biome`、`World Time`、`Real Time`、`Rotation`）、`CustomTextHud`、`BPS`、`MTF`、`Inventory`、`ContainerHUD`、`ModuleList`、`Potions`、`ScaffoldBlock`、`TargetHUD`、`Watermark`、`ArmorHUD`、`ItemCounterHUD`、`CompassHUD`、`HoleHUD`、`LagNotifierHUD`、`PlayerRadarHUD`、`KeyboardHUD`、`PlayerModelHUD`、`MapHUD`） |
| `events/` | 自定义事件总线与事件类型 |

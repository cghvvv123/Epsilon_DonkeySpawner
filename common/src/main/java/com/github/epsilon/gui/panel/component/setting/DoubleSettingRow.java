...
public class DoubleSettingRow extends SettingRow<DoubleSetting> {
    ...
    public void buildUi(UiTree.Scope scope, GuiGraphicsExtractor guiGraphics, TextRenderer textRenderer, UiRect bounds, float hoverProgress, int mouseX, int mouseY, float partialTick) {
        ...
        float displayX = focused && displaySlice != null && (displaySlice.start() > 0 || displaySlice.end() < getDisplayBuffer().length())
                ? fieldBounds.x() + FIELD_TEXT_PADDING
                : fieldBounds.x() + (fieldBounds.width() - displayWidth) / 2.0f;
    ...
    private boolean replaceSelection(String value) {
